...
Patterns such as `Rc<RefCell<T>>` are highly discouraged. This is a very easy way to enter the dreaded "type hell." Only do this if you are in a frontend and are practically forced to do this. For example, if your frontend library (Slint) forced you to use callbacks to handle application events and you have to share state between your application and your UI (beyond just simple status changes), and if there is literally no way around this.
