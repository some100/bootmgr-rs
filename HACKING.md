...
This is a boot manager library that comes with multiple frontends to that library. The primary way of building, testing, and fuzzing this library is through using `cargo xtask`. If you already have the Cargo toolchain installed, run these commands:
```sh
...
The private library side of this codebase runs unit tests with the standard Rust test runner. To run clippy, unit tests, and doctests, run the command:
```sh
cargo xtask test
```

In order to run integration tests (which test public parts of the library), run the command:
```sh
